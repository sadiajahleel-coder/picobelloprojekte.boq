...
### Phase 11 — Import "Validation Failed" Root-Caused, Document Library Aggregation, UI Fixes
User reported BOQ and Historical Project imports failing with a bare "validation failed, 0 imported" and no way to tell why. Root-caused it: `backend/src/middleware/errorHandler.js` already returns field-level detail (`{ message: 'Validation failed', errors: [{ field, message }] }`) for Mongoose validation errors, but `runImport.js` was only reading `.message` — throwing away the one part that actually says what's wrong. Fixed, plus two contributing gaps found while digging in:
- **`frontend/src/utils/runImport.js`** now includes the field-level detail (e.g. "Validation failed: quantity — Path `quantity` is required.") and runs rows in bounded-concurrency batches of 6 instead of one at a time — also answers the "Master Import takes too long" report, since imports were previously fully sequential (one network round-trip at a time).
- **Every `*_IMPORT_COLUMNS` list now marks `required: true` on the fields the backend schema actually requires**, not just the first column (`ExcelImport.jsx`/`MasterImport.jsx` previously only checked/flagged the first column, so e.g. BOQ's `unit`/`quantity`/`baseCost` — all required — showed no warning if left unmapped, silently failing every row). The column-mapping UI now marks every required field and lists all unmapped ones.
- **`frontend/src/utils/enumMatch.js`** — strict backend enums (`condition: 'semi_finished'`, `tier: 'mid_range'`, etc.) don't match what a person actually types in a spreadsheet ("Semi-Finished", "Mid-Range") or copies from this app's own dropdown labels ("Finished (Facelift)"). Normalizes and fuzzy-matches these before submission, wired into every enum-bearing import column across QS/Artisan/Material Pricing, Contacts, Historical Projects, Projects, Change Orders, Progress Tracker, and Invoices.
- **Document Library now aggregates files from across the app**, not just its own "Add Document" records — `backend/src/utils/documentAggregator.js` pulls in Historical Project attachments, Expense receipts, Progress Tracker photos, and Site Report photos as read-only entries (can't be deleted from here, since they belong to their source record), each labeled by where it came from.
...
### Phase 13 — Import Value Remapping, Service Worker 404 Fix
User's real Historical Projects file confirmed headers matched fine (they used the downloaded template) but the *values* they'd typed for Condition/Tier ("New Build", "Tier 1 - Luxury", "Tier 3 - Public") had no lexical overlap with the backend's enum keys at all, so `enumMatch.js`'s fuzzy matching correctly left them unmatched — no amount of synonym-guessing generalizes to arbitrary phrasing.
- **Both `ExcelImport.jsx` and `MasterImport.jsx` now have a value-remapping step**: for any mapped column with `enumValues`, the distinct raw values actually present in the file are listed with a dropdown (defaulting to the auto-guess, highlighted red when unmatched) so the user corrects "Tier 1 - Luxury" → Premium once, rather than the row failing at submit time with no way to fix it in the UI. Column definitions can optionally supply `enumLabels` for a human-readable option list (added for Historical Projects' condition/tier)
- **Root-caused a second, more fundamental cause of the "Not Found" reports**: `frontend/public/sw.js`'s service worker was doing stale-while-revalidate for every page navigation, including ones outside its small pre-cache list (`/`, `/login`, `/register`, `/app/dashboard`). Any other route — which is most of the app — fell through to a real network fetch of that exact path on first visit or cache miss; if the network response wasn't OK (e.g. the host not rewriting unknown paths to `index.html`), the service worker faithfully handed that error response straight to the browser instead of falling back to the cached app shell. Fixed: navigation requests now always resolve to the cached `/` shell when the network response isn't ok or the request fails, letting react-router take over client-side regardless of what the host does with that exact path. This is a real, permanent fix independent of whatever Render's dashboard config turns out to be
- Investigated "sign-in is slow": the login code path itself is lean (one `User.findOne` + `bcrypt.compare`, no synchronous email/upload work). Couldn't verify the live server directly — this session's outbound network is proxied and `squaremetre-boq.onrender.com` returns 403 from the proxy. Bcrypt is configured at cost-factor 12 (normal, but genuinely slower on shared/low-vCPU hosting); the other common cause is a Render free/starter-tier cold start after inactivity. Neither is a code bug to fix here — flagged for the user to check directly (does only the *first* request after idle time feel slow, or every one?)

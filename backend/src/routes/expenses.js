...
// zodValidate runs after multer so multipart text fields have already landed in req.body
router.post('/', authorize('admin', 'qs', 'project_manager'), uploadLimiter, upload.array('receipts', 10), zodValidate(schemas.expense), create);
router.put('/:id', authorize('admin', 'qs', 'project_manager'), uploadLimiter, upload.array('receipts', 10), zodValidate(schemas.expenseUpdate), update);
